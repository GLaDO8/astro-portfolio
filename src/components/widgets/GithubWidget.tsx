...
function cellRadius(row: number, col: number): string {
	...
	const tr = row === 0 && col === COLS - 1 ? "rounded-tr-[8px]" : "rounded-tr-[4px]";
	const bl = row === ROWS - 1 && col === 0 ? "rounded-bl-[8px]" : "rounded-bl-[4px]";
	const br = row === ROWS - 1 && col === COLS - 1 ? "rounded-br-[8px]" : "rounded-br-[4px]";
	return `${tl} ${tr} ${bl} ${br}`;
}
	...
	return (
		<div className="flex flex-col gap-1">
			<div className="flex gap-2 items-center">
				<span className="font-sans font-medium text-sm tracking-[-0.28px] text-[#353535]">
					{repo.name}
				</span>
				<span className="bg-[#f2f2f2] rounded-2xl px-2 pt-px pb-0.5 font-sans font-medium text-xs tracking-[-0.24px] text-[#6a6a6a]">
					Private
				</span>
			</div>
			<span className="font-sans text-sm tracking-[-0.28px] text-[#737373]">
		...
		<div className="w-[230px] shrink-0 bg-white rounded-2xl shadow-lg flex flex-col gap-4 items-center pt-3 pb-4 px-3">
			...
			<div className="flex flex-col gap-3 w-full">
				<div className="flex items-center gap-2.5 w-full">
					<span className="flex-1 font-sans font-bold text-xl tracking-[-0.4px] text-black">
						Sneak peek
					</span>
					<a
						href={GITHUB_PROFILE}
