...
function getSnapGalleryHeight(item: SnapsGalleryItem) {
	...
	if (ratio > 1.6) {
		return "h-[min(clamp(18rem,44dvh,45rem),var(--snaps-image-max-height))] lg:h-[min(45rem,var(--snaps-image-max-height))]";
	}
	if (ratio > 1) {
		return "h-[min(clamp(16rem,38dvh,36rem),var(--snaps-image-max-height))] lg:h-[min(36rem,var(--snaps-image-max-height))]";
	}
	return "h-[min(clamp(12rem,28dvh,24rem),var(--snaps-image-max-height))] lg:h-[min(24rem,var(--snaps-image-max-height))]";
}
...
export default function SnapsGallery({ snaps, priorityCount = 0 }: SnapsGalleryProps) {
	...
	return (
		<motion.div
			ref={galleryRef}
			className="flex h-full min-h-0 w-full items-center gap-x-10 overflow-x-scroll overflow-y-hidden overscroll-x-contain px-5 py-6 sm:gap-x-14 sm:px-8 md:gap-x-20 md:px-16 lg:gap-x-24 lg:px-24 [@media(max-height:860px)]:pt-3 [@media(max-height:860px)]:pb-9 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
			data-snaps-grid=""
			data-lenis-prevent=""
			initial={false}
		>
			{snaps.map((item, itemIndex) => {
				...
				return (
					<motion.figure
						key={item.src}
						className={cn(
							"rounded-sm flex-none border-8 border-white bg-white shadow-[0_12px_34px_-12px_rgb(42_35_29_/_0.42),0_4px_16px_-8px_rgb(42_35_29_/_0.24),0_1px_3px_rgb(42_35_29_/_0.1)] sm:border-12",
							randomRotation(),
						)}
