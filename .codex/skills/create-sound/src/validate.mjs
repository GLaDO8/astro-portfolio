...
import { parseFrontmatter, prefixOf, RULES_DIR, readRuleFiles, readSections } from "./lib.mjs";

const REQUIRED = ["title", "impact"];
const ALLOWED_IMPACTS = new Set(["CRITICAL", "HIGH", "MEDIUM-HIGH", "MEDIUM", "LOW-MEDIUM", "LOW"]);
...
for (const file of ruleFiles) {
	...
	if (data.example) {
		const exampleErrors = validateExample(data.example, file);
		errors.push(...exampleErrors);
	} else if (prefix === "event" || prefix === "mood" || prefix === "layer" || prefix === "effect") {
		warnings.push(
...
console.log(`validate: ${ruleFiles.length} rule(s) ok (${warnings.length} warning(s)).`);

function validateExample(example, file) {
	let parsed;
	try {
